...
function ControlGroup(h, b, g) {
    ...
    this.isLegalCallback = g || function() {
        return true
    }
    ;
...
ControlGroup.prototype.turn = function(b) {
    var e, c = null, g = b ? "cw" : "ccw", f = this.kickOffsets[this.dir][g], d;
...
ControlGroup.prototype.fall = function() {
    var e = this.getFallPositions(), b = e.positions, f = e.dist, d, c;
    if (f !== 0) {
        this.lastWasSpin = false
    }
    for (d = 0; d < 4; d += 1) {
...
ControlGroup.prototype.configurePreviewBlocks = function(c) {
    var b = this.getFallPositions().positions, d;
    for (d = 0; d < 4; d += 1) {
...
var Background = function(c) {
    ...
    this.backdrop = new jaws.Sprite({
        image: "media/background/backdrop.png"
    });
    this.backdrop.x = 0;
    this.backdrop.y = 0;
    this.topBar = new jaws.Sprite({
        image: "media/background/topbar.png"
    });
    this.topBar.x = 181;
    this.topBar.y = 0;
...
ScoreTracker.prototype.updateScore = function(f) {
    ...
    if (this.linesRemaining <= 0) {
		if(document.getElementById("infiniteCheck").checked) {
			if (this.level < 99) { //controls how long the game lasts
				this.level += 1;
				this.linesRemaining = ScoreTracker.levelLines(this.level)
			} else {
				this.isGameWon = true
...
function Game(e, f, d) {
    ...
    this.previewBlocks = [];
    for (b = 0; b < 4; b += 1) {
        this.previewBlocks.push(new Block({
            blockX: -10,
            blockY: -10,
            preview: true
        }))
...
Game.prototype.newBlock = function(e) {
    var d = this, b = this.randBag.popQueue(), f = [], g, c;
    this.dropPeriod = this.scoreTracker.getLevelPeriod();
    for (c = 0; c < 4; c += 1) {
        g = new Block({
            blockX: -10,
...
Game.prototype.swap = function() {
    var d, c, b, h = this.controlGroup.getShape(), g = this.controlGroup.getBlocks(), f = [], e = this;
    if (!this.swapAllowed) {
        return
    }
...
Game.prototype.lockBlocks = function() {
    var b = this.controlGroup.getTSpin(), c = {}, d;
    if (b === "mini") {
        c.miniT = true
    } else {
        if (b === "normal") {
...
function Tetris(d) {
    ...
    this.setup = function() {
        ...
        g = new Button({
            image: "media/buttons/continue.png",
            x: 234,
            y: 165
        });
        b = new Button({
            image: "media/buttons/restart.png",
            x: 234,
            y: 215
        });
        h = new Background();
    ...
    ;
    this.update = function() {
        var u = (new Date()).getTime(), t = jaws.pressed("esc"), s;
        if (f === null) {
            m = 0;
            f = u
        } else {
